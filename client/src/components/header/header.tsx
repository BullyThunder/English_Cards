...
const navLinks = [
    { title: 'Chronology', href: '/timeline' },
    { title: 'Map', href: '/map' },
    ...
    <div className="flex items-center"> 
        ...
        <nav className="hidden md:flex">
            ...
            <div className="flex items-center space-x-4"> 
                {navLinks.map((link) => (
                    <Link key={link.title} href={link.href} className="px-3 py-2 rounded-md text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white transition duration-150">
                        {link.title}
                    </Link>
                ))}
            </div>
        ...
        <div className="md:hidden ml-2"> {/* Добавляем отступ, чтобы не сливалась с иконкой User */}
            {/* ... код кнопки мобильного меню, использующий toggleMenu ... */}
        </div>
        
    </div> {/* Конец Контейнера для навигации + иконки пользователя */}
